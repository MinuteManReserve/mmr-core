...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("minutemanreserve"))
        return false;
...
{
    // Convert minutemanreserve:// to minutemanreserve:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("minutemanreserve://"))
    {
        uri.replace(0, 12, "minutemanreserve:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MinuteManReserve.lnk";
}
...
{
    return GetAutostartDir() / "minutemanreserve.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "Name=MinuteManReserve\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MinuteManReserve-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  minutemanreserve-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("MinuteManReserve-Qt"));
